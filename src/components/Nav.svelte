...
<nav class="{dashed ? "dashed" : undefined}">
    ...
    <ul>
        {#each menus as menu}
            <li class="nav-link" on:click={menu.onclick ? menu.onclick : undefined} link-content="{menu.contents ? "true" : null}"  on:mousemove="{menu.contents ? (e) => menuHover(e, menu) : undefined}">{menu.title}</li>
        {/each}
    </ul>
    ...
    <div class="res-menu">
      ...
      <div>
        ...
        <label for="t{i}" class="title {!menu.contents ? "single-title" : ""}" on:click="{menu.onclick ? menu.onclick : () => null}">{menu.title}</label>
        {#if menu.contents}
...
<div class="menuCard" bind:this="{menuCard}" style="height:{cardHeight}px;left:{cardPosition.x}px;{cardVisible ? "opacity: 1;pointer-events: auto;" : ""}">
